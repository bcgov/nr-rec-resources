...
describe('the FilterGroup component', () => {
  ...
  it('renders a filter group with a label and options', () => {
    ...
    expect(screen.getByText('Snowmobiling (9)')).toBeInTheDocument();
    expect(screen.getByText('Angling (14)')).toBeInTheDocument();
    expect(screen.getByText('Canoeing (5)')).toBeInTheDocument();
    expect(screen.getByText('Camping (12)')).toBeInTheDocument();
  });
