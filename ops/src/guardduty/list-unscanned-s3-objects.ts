...
async function main(): Promise<void> {
  ...
  const { bucket, profile, output, region } = parseArgs({ demandKeysFile: false });
  const resolvedRegion = region ?? process.env.AWS_REGION ?? 'us-east-1';
  const logger = createLogger('list-unscanned', { bucket, region: resolvedRegion });
  ...
  const checks = await runInBatches(
    ...
    async (key) => {
      ...
      if (checked % PROGRESS_INTERVAL === 0) {
        logger.info('Tag check progress', { checked: String(checked), total: String(allKeys.length) });
      }
      return result;
    },
  );
  ...
  if (output && unscanned.length > 0) {
    ...
    logger.info('Wrote keys to file', { count: String(unscanned.length), output });
  }
  logger.info('Completed', { durationMs: String(Date.now() - startMs) });
}
