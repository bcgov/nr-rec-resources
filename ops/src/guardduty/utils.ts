    ...
    builder = builder
      ...
      .check((argv) => {
        ...
        if (typeof k !== 'string' || !k.trim()) {
          throw new Error(
            'keys-file is required and must be a non-empty path (--keys-file). Example: make verify BUCKET=my-bucket KEYS_FILE=unscanned-keys.txt',
          );
        }
        return true;
      });
...
export function runWithFatalHandler(
  scriptName: string,
  main: () => Promise<void>,
): void {
  main().catch((err) => {
