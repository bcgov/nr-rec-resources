...
async function main(): Promise<void> {
  ...
  const { bucket, keysFile, profile, region } = parseArgs({ demandKeysFile: true });
  ...
  logger.info('Verifying objects in bucket', { count: String(keys.length), bucket });
  ...
  const checks = await runInBatches(
    keys,
    CONCURRENCY,
    async (key) => hasScanTag(client, bucket, key, onTagError),
  );
  ...
  if (withoutTag.length > 0) {
    logger.error('Keys still missing the scan tag', { count: String(withoutTag.length), keys: withoutTag });
    withoutTag.forEach((k) => logger.warn('Key missing tag', { key: k }));
    logger.info('Completed', { durationMs: String(durationMs) });
    process.exit(1);
  }

  logger.info('All keys have the GuardDuty scan tag', { count: String(keys.length) });
  logger.info('Completed', { durationMs: String(durationMs) });
}
