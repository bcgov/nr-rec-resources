...
const SHAPEFILE_ALLOWED_EXTENSIONS = ['shp'] as const;
...
export async function readSpatialFile(
  ...
  const source = await shapefile.openShp(arrayBuffer);
  const features: any[] = [];
  ...
  features.forEach((feature, index) => {
    ...
    if (!checkExtentWithinBc(feature)) {
      issues.push({
        type: 'GEOMETRY',
        severity: 'ERROR',
        message: `Feature #${index + 1} lies outside BC bounds (EPSG:3005 extent check).`,
      });
    }
  });
