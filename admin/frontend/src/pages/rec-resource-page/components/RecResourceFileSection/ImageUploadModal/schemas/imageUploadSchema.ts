...
export const createImageUploadSchema = ({
  ...
  const withConsent = disableConsentFields
    ? baseSchema
    : baseSchema
        ...
        .refine(
          (data) => {
            ...
            if (!isStaff) {
              return (
                !!data.photographerName && data.photographerName.trim() !== ''
              );
            }
            return true;
          },
  ...
  const withConfirmation = disableConsentFields
    ? withConsent
    : withConsent.refine((data) => data.confirmationChecked, {
...
export const imageUploadSchema = createImageUploadSchema();
