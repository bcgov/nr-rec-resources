...
export function useAdminSearchController(search: AdminSearchRouteState) {
  ...
  const pageSize = resultsQuery.data?.page_size ?? 25;
  ...
  const appliedFilterChips = useMemo<AdminAppliedFilterChip[]>(
    () =>
      [
        search.q
          ? {
        ...
        search.establishment_date_to
          ...
          : undefined,
      ].filter((chip): chip is AdminAppliedFilterChip => Boolean(chip)),
    [
      accessOptions,
      activityOptions,
      clearAccess,
