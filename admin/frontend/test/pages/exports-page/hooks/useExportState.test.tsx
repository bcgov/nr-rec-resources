...
describe('useExportState', () => {
  ...
  it('resets filters on dataset change and disables preview for unavailable datasets', () => {
    ...
    act(() => {
      result.current.filters.handlers.setSelectedDistrict('RDKA');
      result.current.filters.handlers.setSelectedResourceType('SIT');
    });

    act(() => {
      result.current.filters.handlers.handleSelectDataset('objective-list');
    });
    ...
    act(() => {
      result.current.filters.handlers.handleSelectDataset('file-details');
    });
