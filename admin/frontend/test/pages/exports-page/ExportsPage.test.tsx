...
describe('ExportsPage', () => {
  ...
  it('renders unavailable datasets as disabled options and shows the warning state', () => {
    ...
    expect(
      screen.getByText(
        'Choose a dataset to automatically load up to 50 rows from the backend. District and type changes refresh the preview.',
      ),
    ).toBeInTheDocument();
  });
  ...
  it('renders the default guidance and preview values, including null fallbacks', () => {
    ...
    expect(screen.getByText('Source: RST')).toBeInTheDocument();
  });
  ...
  it('calls filter and download handlers from user actions', () => {
    ...
    const setSelectedResourceType = vi.fn();
    ...
    expect(handleSelectDataset).toHaveBeenCalledWith('file-details');
    expect(setSelectedDistrict).toHaveBeenCalledWith('RDKA');
    expect(setSelectedResourceType).toHaveBeenCalledWith('SIT');
