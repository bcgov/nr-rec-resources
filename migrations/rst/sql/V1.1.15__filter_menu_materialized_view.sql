...
from
  ...
  left join recreation_map_feature rmf on rmf.recreation_resource_type = rrtc.rec_resource_type_code
  left join recreation_resource rr on rr.rec_resource_id = rmf.rec_resource_id
where
  rrtc.rec_resource_type_code not in ('RR')
  and rr.display_on_public_site = true
group by
  rmf.recreation_resource_type,
  rrtc.description
order by
...
left join rst.recreation_resource_type_code rrtc on rmf.recreation_resource_type = rrtc.rec_resource_type_code  -- Join to get the type description
group by rr.rec_resource_id, rmf.recreation_resource_type, rs.status_code, rsc.description, rs.comment, rrtc.description;
