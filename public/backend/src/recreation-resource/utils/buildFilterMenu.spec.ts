...
describe('buildFilterMenu', () => {
  ...
  describe('when provided with partial data', () => {
    it('should handle missing categories', () => {
      ...
      expect(result).toHaveLength(7); // Should still return all filter categories
      ...
      expect(result[3].options).toHaveLength(0); // Empty fees
      expect(result[4].options).toHaveLength(0); // Empty district
      expect(result[5].options).toHaveLength(2); // Facilities (always shows both - tables and toilets)
      expect(result[6].options).toHaveLength(0); // Empty access
    });
