...
TLS_CLIENT_PROTOCOLS="${JDK_TLS_CLIENT_PROTOCOLS:-TLSv1.2}"
JAVA_OPTS="-Djdk.tls.client.protocols=${TLS_CLIENT_PROTOCOLS} -Duser.name=REC-ORACLE-EL -Xms512m -Xmx512m -XX:TieredStopAtLevel=4 -XX:+UseParallelGC -XX:MinHeapFreeRatio=20 -XX:MaxHeapFreeRatio=40 -XX:GCTimeRatio=4 -XX:AdaptiveSizePolicyWeight=90 -XX:MaxMetaspaceSize=350m -XX:+ExitOnOutOfMemoryError"
# if debug ssl handshake is true add -Djavax.net.debug=ssl:handshake else not needed.
if [ -n "${SSL_DEBUG}" ]; then
  java -Djavax.net.debug=ssl:handshake ${JAVA_OPTS} -jar app.jar
else
