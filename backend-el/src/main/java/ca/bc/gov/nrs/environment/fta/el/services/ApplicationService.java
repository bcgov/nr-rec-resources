...
public class ApplicationService {
  ...
  private void extractAndUploadRecreationAttachmentContent(final List<RecreationAttachment> recAttachments) {
    var isAttachmentProcessCompletedOnceEarlier = this.s3UploaderService.isAttachmentProcessCompletedEarlier();
    if (isAttachmentProcessCompletedOnceEarlier) {
      // find the recAttachments of last 2 weeks and run through them.
      var recAttachmentsOfLastTwoWeeks = recAttachments.stream()
        .filter(recAttachment -> recAttachment.getUpdateTimestamp()
          .isAfter(LocalDateTime.now().minusWeeks(2)))
        .toList();
      logger.info("total attachments to be processed : {}", recAttachmentsOfLastTwoWeeks.size());
      if (!recAttachmentsOfLastTwoWeeks.isEmpty()) {
        processRecAttachmentsAndUploadToS3IfNotPresent(recAttachmentsOfLastTwoWeeks);
      }
    } else {
      processRecAttachmentsAndUploadToS3IfNotPresent(recAttachments);
    }

  }

  private void processRecAttachmentsAndUploadToS3IfNotPresent(List<RecreationAttachment> recAttachments) {
